#Федосеев Алексей, Вариант 7
#Задание 2. Cоздайте модель множественной линейной регрессии дневных  
#потоков паров воды за летний период 2013 года по данным измерений 
...
eddypro = filter(eddypro, DOY >= 181 & DOY < 243)
...
mod2 = lm(h2o_flux~ DOY + Tau + qc_Tau + rand_err_Tau + H + qc_H
          ...
          + rand_err_H + LE + qc_LE + rand_err_LE + rand_err_co2_flux
           ...
           + H_strg + h2o_v_minus_adv
          
           + co2_molar_density + co2_mole_fraction
          
           + air_temperature
          
           + air_molar_volume + water_vapor_density + e + es
          
           + Tdew + u_unrot + v_unrot + w_unrot + u_rot + v_rot
          
           + yaw + pitch + u_star_ + TKE + L
          
          + x_10_perc_ + x_30_perc_ + x_50_perc_ + x_70_perc_ + x_90_perc_ + un_Tau
          
           + un_LE + LE_scf + un_co2_flux + u_spikes + ts_var
...
mod3 = lm(co2_flux~ DOY + Tau + qc_Tau + rand_err_Tau + H + qc_H
          ...
          + rand_err_H + LE + qc_LE + rand_err_LE + rand_err_co2_flux
           ...
           + h2o_flux + rand_err_h2o_flux + H_strg + h2o_v_minus_adv
          
           + h2o_mole_fraction + h2o_mixing_ratio + co2_molar_density + co2_mole_fraction
          
           + h2o_time_lag + sonic_temperature + air_temperature
          
           + air_density + air_heat_capacity + air_molar_volume + water_vapor_density + e + es
          
           + RH + VPD + Tdew + u_unrot + v_unrot + w_unrot + u_rot + v_rot
          
           + max_speed + wind_dir + yaw + pitch + u_star_ + TKE + L
          
           + x_peak + x_offset + x_10_perc_ + x_30_perc_ + x_50_perc_ + x_90_perc_ + un_Tau
          
           + un_H + H_scf + un_LE + LE_scf + un_co2_flux + u_spikes + ts_var
...
cor_teaching_tbl = select(teaching_tbl, h2o_flux, DOY, Tau, qc_Tau, rand_err_Tau, H, qc_H, rand_err_H,
